...
class Filmweb(object):
    ...
    def get_dataframe(self, df, use_saved_scraped=False):
        df.columns = FILMWEB_DATA_COLUMNS
        ...
        df = df.join(self._dummies(df['Gatunek']), how='outer')
        df = df.join(self._dummies(df['Kraj produkcji']), how='outer')
        df = df.join(self._get_scrapped(df, use_saved_scraped), how='outer')
